...
describe(utils.suiteName(__filename), function () {
  ...
  describe("logscale", function () {
    ...
    it("should parse string logscale (using explicit base)", async function () {
      let options =
        (await Chart._parseDrawInputs([], { logscale: ["cb", "2"] }))[1];
      assert.equal(options.logscale.axes, "cb");
      assert.equal(options.logscale.base, 2);

      options =
        (await Chart._parseDrawInputs([], { logscale: ["xyy2", 6] }))[1];
    ...
    it("should reject bases that are not numbers >= 1", async function () {
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: ["xyy2", -4] })
      );
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: ["xyy2", "-9"] })
      );
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: ["xyy2", "0"] })
      );
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: ["xyy2", "0.5"] })
      );
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: ["xyy2", "0.99"] })
      );
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: ["xyy2", "h"] })
      );
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: ["xyy2", []] })
      );
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: ["xyy2", ["hello"]] })
      );
    });
    it("should reject invalid axes spec", async function () {
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: ["xyz2", 5] })
      );
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: ["h", 5] })
      );
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: ["  ", 5] })
      );
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: [[], 5] })
      );
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: [["x"], 5] })
      ); // don't allow arrays

      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: "xyz2" })
      );
      await assert.rejects(() => Chart._parseDrawInputs([], { logscale: "h" }));
      await assert.rejects(() =>
        Chart._parseDrawInputs([], { logscale: "  " })
      );
    ...
    return chart.draw(lines, opts)
      .then(() => {
        ...
        const expected = fs.readFileSync(
          path.join(__dirname, "xTicks-isTimeSeries.png"),
        );

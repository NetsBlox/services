...
describe(testUtils.suiteName(__filename), function () {
  ...
  describe("encodeQueryData", () => {
    it("should not ignore null values", () => {
      let encodedQuery = utils.encodeQueryData({
        query: "test",
        secret: null,
        fieldB: undefined,
      });
  ...
  describe("snap structure creation", function () {
    ...
    const stringJson =
      '{"name":"Lisa","age":"2","gholi":"foo","friends":[{"avtar":true},23,45,56]}';
    ...
    it("should convert date objects to GMT datestring", function () {
      assert(
        utils.jsonToSnapList(multipleData)[2][3][1] ===
          "Tue, 25 Jul 2017 22:49:04 GMT",
      );
    });
  });
});

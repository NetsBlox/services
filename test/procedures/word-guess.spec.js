...
describe(utils.suiteName(__filename), function () {
  ...
  it("giveUp should return word and end game", function () {
    ...
    assert.equal(
      wordguess.giveUp(),
      WordGuess._states[Object.keys(WordGuess._states)[0]].word,
    );
    assert.equal(
      WordGuess._states[Object.keys(WordGuess._states)[0]].gamestate,
      WordGuess._GameState.Lost,
    );
  ...
  it("bad guess should throw", function () {
    ...
    // Invalid word
    assert.throws(() => {
      wordguess.guess("aaaaa");
    }, RPCError);

    // Invalid word
    assert.throws(() => {
      wordguess.guess("12345");
    }, RPCError);

    // Valid attempt
    assert.doesNotThrow(() => {
      wordguess.guess("apple");
    });

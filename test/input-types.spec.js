...
describe(utils.suiteName(__filename), function () {
  ...
  describe("BoundedInteger", function () {
    ...
    it("should not allow non-numbers", async () => {
      await utils.shouldThrow(() =>
        typesParser.BoundedInteger("hello", params)
      );
    });
  ...
  describe("Array", function () {
    ...
    it("should enforce bounded lengths", async () => {
      ...
      await utils.shouldThrow(() =>
        typesParser.Array([1, 2, 3, 4, 5, 6], [undefined, undefined, 4])
      );
      await utils.shouldThrow(() =>
        typesParser.Array([1, 2, 3, 4, 5], [undefined, undefined, 4])
      );
      ...
      await utils.shouldThrow(() =>
        typesParser.Array([1, 2], [undefined, 3, 4])
      );
      await utils.shouldThrow(() =>
        typesParser.Array([1, 2, 3, 4, 5], [undefined, 3, 4])
      );
  ...
  describe("Tuple", function () {
    ...
    it("should throw on wrong arity input", async () => {
      ...
      assert.deepStrictEqual(await typesParser.SomeTupleB(["12", "hello"]), [
        12,
        "hello",
      ]);
      await utils.shouldThrow(() =>
        typesParser.SomeTupleB(["12", "hello", []])
      );
    });
    it("should accept only correct type sequence", async () => {
      await utils.shouldThrow(() =>
        typesParser.SomeTupleC(["12.4", [], "hello"])
      );
      await utils.shouldThrow(() =>
        typesParser.SomeTupleC(["12", "67", "hello"])
      );
      await utils.shouldThrow(() =>
        typesParser.SomeTupleC(["12", "67", "hello"])
      );
      assert.deepStrictEqual(
        await typesParser.SomeTupleC(["12", ["finally"], "hello"]),
        [12, ["finally"], "hello"],
      );
    });
  });
  ...
  describe("Date", function () {
    ...
    const assertDatesEq = (a, b) => {
      ...
      assert(
        Math.abs(+a - +b) < DATE_EPSILON_MS,
        `not (approx) equal: |${+a} - ${+b}| = ${dist}\n(${a} vs ${b})`,
      );
    };
    ...
    it("should support time offsets", async () => {
      ...
      assertDatesEq(await parse("2020/10/3+12d"), new Date("2020/10/15"));
      assertDatesEq(
        await parse(" 2020/10/3  +12d    -2day  "),
        new Date("2020/10/13"),
      );
      assertDatesEq(
        await parse(" 2020/10/3  +12d    -2day -1  week "),
        new Date("2020/10/6"),
      );
      assertDatesEq(
        await parse("  2020/10/3+12days-2d  "),
        new Date("2020/10/13"),
      );
      assertDatesEq(
        await parse("2353436446 +20m -10secs +4000ms"),
        new Date(2353436446 + 20 * 60 * 1000 - 10 * 1000 + 4000),
      );
      assertDatesEq(
        await parse("+2353436446 +20m -10secs +4000  ms"),
        new Date(2353436446 + 20 * 60 * 1000 - 10 * 1000 + 4000),
      );
      assertDatesEq(
        await parse("+  2353436446 +20m -10secs +4000ms"),
      ...
      assertDatesEq(
        await parse("   +2353436446 +20  mins -10secs +4000ms"),
        new Date(2353436446 + 20 * 60 * 1000 - 10 * 1000 + 4000),
      );
      assertDatesEq(
        await parse("   +2353436446+   20m -10secs +4000ms"),
  ...
  describe("Duration", function () {
    ...
    it("should parse chained durations", async () => {
      ...
      assert.deepStrictEqual(
        await parse(" 12d  -  5    hrs  "),
        12 * 24 * 60 * 60 * 1000 - 5 * 60 * 60 * 1000,
      );
      assert.deepStrictEqual(
        await parse(" 5s - 10s + 12ms"),
        5 * 1000 - 10 * 1000 + 12,
      );
    });
  });
  ...
  describe("Object", function () {
    it("should throw error if input has a pair of size 0", async () => {
      let rawInput = [[], ["a", 234], ["name", "Hamid"], ["connections", [
        "b",
        "c",
        "d",
      ]]];
    ...
    it("should throw error if input has a pair of length more than 2", async () => {
      let rawInput = [["a", 234], ["name", "Hamid", "Z"], ["connections", [
        "b",
        "c",
        "d",
      ]]];
    ...
    describe("duck typing", function () {
      ...
      it("should support optional fields", async function () {
        const input = [];
        const parsedInput = await typesParser.Object(input, [
  ...
  describe("BoundedString", function () {
    ...
    it("should throw if less than min", async () => {
      let rawInput = "a";
      const err = await utils.shouldThrow(() =>
        typesParser[type](rawInput, [4, 180])
      );
  ...
  describe("Enum", function () {
    ...
    it("should take an object of variant key value pairs and return mapped value", async () => {
      ...
      assert.deepEqual(await typesParser[type]("puppy", vars), 3);
      assert.deepEqual(await typesParser[type]("KITTEN", vars), [
        "hello",
        "world",
      ]);
      assert.deepEqual(await typesParser[type]("kitteN", vars), [
  ...
  describe("withTypeTape", function () {
    it("should return newly defined types", function () {
      ...
      assert.deepEqual(types.map((t) => t.meta.displayName), [
        "NewType2",
        "NewType3",
      ]);
    });
  ...
  describe("NonProfaneString", function () {
    ...
    it("should prevent profane input", async () => {
      ...
      assert.rejects(async () =>
        typesParser.NonProfaneString("NetsBlox is great!")
      );

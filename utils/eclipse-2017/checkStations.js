...
const fs = require("fs"),
  ...
  stationUtils = require(
    "../../../src/server/rpc/procedures/eclipse-2017/stations.js",
  ),
...
let calcStationStats = () => {
  ...
  return getReadingsCol().aggregate([aggregateQuery]).toArray().then(
    (updates) => {
      ...
      updates.forEach((update) => {
        ...
        let opPromise = getReadingsCol().find(query).sort({ lastReadingAge: 1 })
          .skip(update.count / 2 - 1).limit(1).toArray()
          .then((readings) => {
            let median = readings[0].lastReadingAge;
            // could also do trimmed median?
            let updateObj = {
              $set: {
                readingAvg: update.readingAvg,
                updates: update.count,
                readingMedian: median,
              },
            };
            let updateOne = {
              updateOne: {
    ...
    stations.forEach((station) => {
      let updateOne = {
        updateOne: {
          ...
          update: {
            $set: {
              distance: distanceToPath(station.latitude, station.longitude),
            },
          },
          upsert: false,
        },
...
storage.connect().then(() => {
  ...
  if (process.argv[2] === "pullUpdates") {
    return fireUpdates(stationUtils.selected);
  }
...
if (process.argv.length < 3) {
  console.log("pass in a command: seed, updateStats or pullUpdates");
}

function reqUpdate(id) {
  let url =
    `http://api.wunderground.com/api/${WU_KEY}/conditions/q/pws:${id}.json`;
  return rp({ uri: url, json: true }).then((resp) => {
    if (resp.response.error) {
      throw resp.response.error.description || resp.response.error.type ||
        "wrong pws id";
    }
    ...
    if (delay > 30) {
      console.error(
        `${id} reporting old reading from ${reading.readAt.toISOString()}`,
      );
    }
    return reading;
  });
...
function distanceToPath(lat, lon) {
  ...
  for (var i = 0; i < pathPoints.length; i++) {
    let dist = geolib.getDistance(poi, {
      latitude: pathPoints[i][0],
      longitude: pathPoints[i][1],
    }) / 1000;
    if (dist > min) break;
    min = dist;
  }

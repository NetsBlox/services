...
Storage.prototype.getDatabaseFromURI = function (mongoURI) {
  return mongoURI.replace(/^(mongodb:\/\/)?[a-zA-Z0-9-_:\.]+\/?/, "") ||
    "admin";
};
...
Storage.prototype.connect = async function (mongoURI) {
  ...
  try {
    ...
    console.error(
      "To connect to a different mongo instance, set MONGO_URI to the mongo uri and try again:",
    );
    console.error("");
    console.error(
      "    MONGO_URI=mongodb://some.ip.address:27017/ netsblox start",
    );
    console.error("");
    console.error("or, if running from the root of the netsblox project:");
    console.error("");
    console.error(
      "    MONGO_URI=mongodb://some.ip.address:27017/ ./bin/netsblox start",
    );
    console.error("");

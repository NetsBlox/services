...
class DataService {
  ...
  async _getFunctionForMethod(method, data) {
    if (method.code) {
      ...
      const transformFn = method.transform
        ? await InputTypes.parse.Function(method.transform.code)
        : (row) => row;
      const combineFn = method.combine
        ? await InputTypes.parse.Function(method.combine.code)
        : (list, item) => list.concat([item]);

      return () =>
        async function () {
          const [queryArgs, transformArgs, combineArgs] = this._getArgs(
            method,
            arguments,
          );
      ...
      this._logger.warn(
        `Malformed method ${method.name}. Needs "query" or "code"`,
      );
    }
  }
  ...
  _getArgs(method, allArgs) {
    ...
    const transformArgCount = method.transform
      ? method.transform.arguments.length - 1
      : 0;
    const combineArgCount = method.combine
      ? method.combine.arguments.length - 2
      : 0;
  ...
  async compile(data = this.data, spec = this.spec) {
    if (spec.code) {
      ...
      const transformFn = spec.transform
        ? await InputTypes.parse.Function(spec.transform.code)
        : (row) => row;

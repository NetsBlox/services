...
function parseInlineTags(desc) {
  ...
  for (;;) {
    ...
    assert(
      !parsed.description.length || !parsed.tags.length,
      "An inline tag may not have a description",
    );
...
function simplify(metadata) {
  ...
  let simplifyParam = (param) => {
    ...
    if (type) {
      ...
      logger.warn(
        `rpc ${fnName}, parameter ${name} is missing the type attribute`,
      );
    }
    return simpleParam;
  };
  ...
  let args = tags
    ...
    .reduce((args, arg) => {
      ...
      if (isFieldInObject) {
        ...
        assert(
          objectArg,
          `Could not find ${objectName} argument expected for ${name}`,
        );
        assert.equal(
          objectArg.type.name,
          "Object",
...
function parseSource(source, searchScope) {
  ...
  const rawDescription = serviceAnnotation &&
    serviceAnnotation.parsed.description;
  const description = rawDescription
    ? cleanMarkup(rawDescription)
    : rawDescription;
  let categories = [];
  let tags = [];
    ...
    categories = serviceAnnotation.parsed.tags
      ...
      .map((tag) => {
        ...
        if (tag.description.toUpperCase() === "GLOBAL") {
          if (tag.description !== "GLOBAL") {
            throw Error(
              `Reserved word GLOBAL used with invalid casing (${tag.description})`,
            );
          }
          return [];
        } else {
  ...
  const rpcDocs = blocks
    .filter((block) =>
      !block.parsed.tags.find((tag) => tag.title === "service")
    )
    .filter((block) => {
      let linesToSearch = lines.slice(
        block.endLine,
        block.endLine + searchScope,
      );
      let fnName;
      ...
      if (nameTag) {
        ...
        if (!fnName) {
          logger.warn(
            `can't associate ${block.lines} with any function. # Fix it at line ${block.beginLine}, column ${block.column}`,
          );
          return false;
        }
        block.parsed.tags.push({
          title: "name",
          name: fnName,
          description: null,
        });

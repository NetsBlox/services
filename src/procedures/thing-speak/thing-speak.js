...
const thingspeakIoT = new ApiConsumer(
  "Thingspeak",
...
function getUpdatedSincePred(updatedSince) {
  ...
  return async function (v) {
    const meta = await thingspeakIoT._requestData({
      path: `${v.id}/feeds/last_data_age.json`,
    });
    return now - +meta.last_data_age >= cutoff;
  };
}

thingspeakIoT._paginatedSearch = async function (
  path,
  query,
  limit = 15,
  pred = TRUE,
  ...
  query.page = 1;
  const first = await this._requestData({
    path,
    queryString: rpcUtils.encodeQueryData(query),
  });
  const totalPages = Math.ceil(
    first.pagination.total_entries / first.pagination.per_page,
  );
  ...
  for (let iter = 1; iter <= 128 && query.page <= totalPages; ++iter) {
    ...
    this._logger.info(
      `paginatedSearch - ${final.length} items after iter ${iter} (kept ${
        100 * keptFrac
      }%)`,
    );
  }
 ...
 * @returns {Array<Object>} search results
 */
thingspeakIoT.searchByLocation = function (
  latitude,
  longitude,
  distance = 100000,
  limit = 15,
 ...
 * @returns {Array<Object>} search results
 */
thingspeakIoT.searchByTagAndLocation = function (
  tag,
  latitude,
  longitude,
  distance = 100000,
...
thingspeakIoT.channelDetails = async function (id) {
  const data = await this._requestData({ path: id + ".json" }).catch(
    this._handleError,
  );

...
const REQUIRED_SUPP_OFFENSE = {
  "MVT_RECOVERED": {
    req: "motor-vehicle-theft",
    category: "MVT Recovered",
    offense: "motor vehicle theft",
  },
  "LARCENY_TYPE": {
    req: "larceny",
    category: "Larceny Type",
    offense: "larceny",
  },
};
function assertSupplementalOffenseCat(offense, category) {
  ...
  if (req && offense !== req.req) {
    throw Error(
      `The "${req.category}" category can only be used with the "${req.offense}" offense option`,
    );
  }
}
 ...
 */
Crime.nationalSupplementalCount = async function (
  offense,
  category,
  startYear,
  endYear,
  ...
  const res = await this._requestData({
    path:
      `api/data/supplemental/${offense}/national/${category}/${startYear}/${endYear}`,
    queryString: `api_key=${this.apiKey.value}`,
  });
 ...
 */
Crime.stateSupplementalCount = async function (
  state,
  offense,
  category,
  startYear,
  ...
  const res = await this._requestData({
    path:
      `api/data/supplemental/${offense}/states/${state}/${category}/${startYear}/${endYear}`,
    queryString: `api_key=${this.apiKey.value}`,
  });
  // res has pagination etc. etc. see other comments
  return res.results;
 ...
 */
Crime.nationalArrestCount = async function (
  offense,
  category,
  startYear,
  endYear,
 ...
 */
Crime.regionalArrestCount = async function (
  region,
  offense,
  category,
  startYear,
  ...
  const res = await this._requestData({
    path:
      `api/arrest/regions/${region}/${offense}/${category}/${startYear}/${endYear}`,
    queryString: `api_key=${this.apiKey.value}`,
  });
...
Crime.stateArrestCount = async function (state, category, startYear, endYear) {
  ...
  const res = await this._requestData({
    path:
      `api/arrest/states/offense/${state}/${category}/${startYear}/${endYear}`,
    queryString: `api_key=${this.apiKey.value}`,
  });

...
chart._prepareData = function (input, options = defaults) {
  ...
  if (
    ...
    !Array.isArray(input[0][0]) && input[0].length === 2
  ) {
...
function isValidAxesString(axes) {
  return (axes.match(AXES_REGEX) || []).reduce((a, v) => a + v.length, 0) ===
    axes.length;
}
async function parseLogscale(options) {
  ...
  if (typeof val === "string") res = { axes: val, base: 10 };
    ...
    if (typeof axes !== "string") {
      throw Error("logscale axes was not a string (text)");
    }
    if (base !== undefined) {
      try {
  ...
  if (!isValidAxesString(res.axes)) {
    throw Error("axes must be a combination of axis names like x or y");
  }
  return res;
}
...
chart._parseDrawInputs = async function (lines, options) {
  ...
  if (options.yRange.length === 2) {
    opts.yRange = { min: options.yRange[0], max: options.yRange[1] };
  }

  if (!options.isCategorical) {
    relativePadding.x = stats.x.range !== 0 ? stats.x.range * 0.05 : 1;
    opts.xRange = {
      min: stats.x.min - relativePadding.x,
      max: stats.x.max + relativePadding.x,
    };
  ...
  if (options.xTicks) {
    if (options.isCategorical) {
      throw Error("can't change the number of xTicks in categorical charting");
    }

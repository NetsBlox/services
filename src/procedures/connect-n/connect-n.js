...
ConnectN.prototype.newGame = function (row, column, numDotsToConnect) {
  ...
  this._state.numDotsToConnect = Math.min(
    Math.max(this._state.numRow, this._state.numCol),
    this._state.numDotsToConnect,
  );
...
ConnectN.prototype.play = async function (row, column) {
  ...
  const winnerId = ConnectN.getWinner(
    this._state.board,
    this._state.numDotsToConnect,
  );
...
ConnectN.getWinner = function (board, numDotsToConnect) {
  ...
  possibleWinners.push(
    ConnectN.getHorizontalWinner(rotatedBoard, numDotsToConnect),
  );
  ...
  possibleWinners.push(
    ConnectN.getDiagonalWinner(board, numDotsToConnect) ||
...
ConnectN.getDiagonalWinner = function (board, numDotsToConnect) {
  ...
  var res;
  for (j = 0, i = 0; j < col; j++) {
    res = ConnectN.getDiagonalWinnerFromStartPoint(
      board,
      numDotsToConnect,
      i,
      j,
...
ConnectN.getDiagonalWinnerFromStartPoint = function (
  board,
  numDotsToConnect,
  i,
  j,

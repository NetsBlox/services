 ...
 *
 * @param {Number} arcHour
 * @param {Number} arcMin
 * @param {Number} arcSec
 */
StarMap.arcHourMinSecToDeg = function (arcHour, arcMin, arcSec) {
  return 360.0 * (1.0 / 24.0) *
    ((1.0 / 60.0) * (((1.0 / 60.0) * parseFloat(arcSec)) + parseFloat(arcMin)) +
      parseFloat(arcHour));
};
 ...
 * @param {String} name
 */
StarMap.findObject = function (name) {
  var rsp = this.response;
  var queryUrl =
    `http://skyserver.sdss.org/dr13/en/tools/Resolver.ashx?name=${name}`;
 ...
 * @param {Number} arcseconds_per_pixel
 * @param {String} options
 * @param {Number=} width
 * @param {Number=} height
 */
StarMap.getImage = function (
  right_ascension,
  declination,
  ...
  if (isNaN(scale) || scale > 32.0 || scale < 1.0 / 128.0) {
    rsp.status(400).send(
      "Error: arcseconds_per_pixel is out of range. Valid values are between 1/128 and 32",
    );
  } else if (!right_ascension || !declination) {
    rsp.status(400).send(
      "Error: right_ascension and declination not specified",
    );
    ...
    url = baseUrl +
      `/ImgCutout/getjpeg?ra=${right_ascension}&dec=${declination}&width=${width}&height=${height}&scale=${scale}&opt=${options}`;

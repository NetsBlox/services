...
async function getStockSymbol() {
  if (
    ...
    Date.now() - CACHE_TIME_STAMP <= DATA_SOURCE_LIFETIME
  ) return CACHED_DATA;

  const resultSet = [];
  const data = await AlphaVantage._requestData({
    path: "query",
    queryString: `function=LISTING_STATUS&apikey=${AlphaVantage.apiKey.value}`,
  });
...
function parseFractionalYear(value) {
  ...
  const frac = (new Date(value) - yearDate) /
    (new Date((year + 1).toString()) - yearDate);
  return year + frac;
}
...
AlphaVantage.equitiesSymbolSearch = async function (search) {
  ...
  for (const element of await getStockSymbol()) {
    if (
      element[1] !== undefined && (element[1].toLowerCase()).includes(search)
    ) {
...
AlphaVantage._rawSearchEquities = async function (
  apifunction,
  symbol,
  interval,
  attributes,
  ...
  if (apifunction == "TIME_SERIES_INTRADAY") {
    data = await this._requestData({
      path: "query",
      queryString:
        `function=${apifunction}&outputsize=full&symbol=${symbol}&interval=${interval}min&apikey=${this.apiKey.value}`,
    });
    ...
    data = await this._requestData({
      path: "query",
      queryString:
        `function=${apifunction}&outputsize=full&symbol=${symbol}&apikey=${this.apiKey.value}`,
    });
    ...
    data = await this._requestData({
      path: "query",
      queryString:
        `function=${apifunction}&symbol=${symbol}&apikey=${this.apiKey.value}`,
    });
  ...
  for (const entry in data[labelAppend]) {
    ...
    const timeseries = [
      (dateFormat === "fractional" ? parseFractionalYear(entry) : t).toString(),
    ];
 ...
 */
AlphaVantage.getEquityData = function (
  symbol,
  interval,
  attributes = "all",
  dateFormat = "traditional",
  startDate,
  endDate,
) {
  ...
  if (apiFunc == undefined) {
    return AlphaVantage._rawSearchEquities(
...
AlphaVantage._rawSearchCrypto = async function (
  apifunction,
  symbol,
  interval,
  market,
  ...
  if (apifunction == "CRYPTO_INTRADAY") {
    data = await this._requestData({
      path: "query",
      queryString:
        `function=${apifunction}&outputsize=full&symbol=${symbol}&interval=${interval}min&market=${market}&apikey=${this.apiKey.value}`,
    });
    ...
    data = await this._requestData({
      path: "query",
      queryString:
        `function=${apifunction}&outputsize=full&symbol=${symbol}&market=${market}&apikey=${this.apiKey.value}`,
    });
    ...
    data = await this._requestData({
      path: "query",
      queryString:
        `function=${apifunction}&symbol=${symbol}&market=${market}&apikey=${this.apiKey.value}`,
    });
 ...
 * @returns {Array<Tuple<Any, Any>>} Array of time series results, which are pairs of ``time`` and ``data``.
 */
AlphaVantage.getCryptoData = function (
  symbol,
  interval,
  attributes = "all",
  dateFormat = "traditional",
  ...
  const apiFunc = apiFuncs[interval];
  if (apiFunc == undefined) {
    return AlphaVantage._rawSearchCrypto(
      "CRYPTO_INTRADAY",
      symbol,
      interval,
      "USD",
...
AlphaVantage._rawSearchForex = async function (
  apifunction,
  fromSymbol,
  toSymbol,
  interval,
  ...
  if (apifunction == "FX_INTRADAY") {
    data = await this._requestData({
      path: "query",
      queryString:
        `function=${apifunction}&outputsize=full&from_symbol=${fromSymbol}&to_symbol=${toSymbol}&interval=${interval}min&apikey=${this.apiKey.value}`,
    });
    ...
    data = await this._requestData({
      path: "query",
      queryString:
        `function=${apifunction}&outputsize=full&from_symbol=${fromSymbol}&to_symbol=${toSymbol}&apikey=${this.apiKey.value}`,
    });
    ...
    data = await this._requestData({
      path: "query",
      queryString:
        `function=${apifunction}&from_symbol=${fromSymbol}&to_symbol=${toSymbol}&apikey=${this.apiKey.value}`,
    });
 ...
 * @returns {Array<Tuple<Any, Any>>} Array of time series results, which are pairs of ``time`` and ``data``.
 */
AlphaVantage.getExchangeData = function (
  fromSymbol,
  toSymbol,
  interval,
  attributes = "all",
  ...
  const apiFunc = apiFuncs[interval];
  if (apiFunc == undefined) {
    return AlphaVantage._rawSearchForex(
      "FX_INTRADAY",
      fromSymbol,
      toSymbol,
      interval,
...
AlphaVantage.convertCurrency = async function (fromSymbol, amount, toSymbol) {
  ...
  const data = await this._requestData({
    path: "query",
    queryString:
      `function=CURRENCY_EXCHANGE_RATE&from_currency=${fromSymbol}&to_currency=${toSymbol}&apikey=${this.apiKey.value}`,
  });
  if ("Error Message" in data) {

...
let ensureNBits = (str, n) => {
  ...
  if (diff < 0) {
    throw new Error(
      `input binary out of the defined alphabet range ${str.length} vs ${n}`,
    );
  }
...
let binaryToAsciiChar = (binaryStr) => {
  if (binaryStr.length !== ASCII_SIZE) {
    throw new Error(`input has to be ${ASCII_SIZE} bits`);
  }
...
let lcs = (xInt, nBits) => {
  if (nBits === undefined) {
    throw new Error("missing input: number of bits to shift is required");
  }
...
let lcsn = (xInt, nBits, unsignedBitCount) => {
  ...
  if (unsignedBitCount > 32 || unsignedBitCount < 1) {
    throw new Error("bad number size");
  }
  let res = (xInt << nBits | xInt >>> unsignedBitCount - nBits) &
    (Math.pow(2, unsignedBitCount) - 1);
  return res;
};
...
let rcsn = (xInt, nBits, unsignedBitCount) => {
  ...
  let res = (xInt << unsignedBitCount - nBits | xInt >>> nBits) &
    (Math.pow(2, unsignedBitCount) - 1);
  return res;
};

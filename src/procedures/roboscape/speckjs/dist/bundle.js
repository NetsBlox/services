    ...
    1: [function (require, module, exports) {
      ...
      class BlockCipher {
        ...
        constructor(n, m, numRounds) {
          ...
          console.log(
            `creating block cipher with block size: ${2 * n} key size: ${
              m * n
            }`,
          );
          this.m = m;
        ...
        _checkKeyWords(keyWords) {
          if (!Array.isArray(keyWords) || keyWords.length !== this.m) {
            throw new Error("bad key words");
          }
          keyWords.forEach((key) => {
            if (isNaN(key) || key > this.MAX_WORD) {
              throw new Error("bad key word", key);
            }
          });
        }
        ...
        _numericBlocksToText(intWords, removeNulls = false) {
          ...
          if (removeNulls) {
            charBitBuckets = charBitBuckets.filter((charBits) =>
              charBits !== NULL_CHAR
            );
          }

          return charBitBuckets.map((binaryChar) =>
            binaryToAsciiChar(binaryChar)
          )
            .join("");
        }
        ...
        encryptAscii(text, keyWords) {
          ...
          blocks.forEach((block) => {
            ...
            if (encX > this.MAX_WORD || encY > this.MAX_WORD) {
              throw new Error("encryptiong returned too big a number");
            }
        ...
        decryptAscii(text, keyWords) {
          ...
          blocks.forEach((block) => {
            ...
            if (decX > this.MAX_WORD || decY > this.MAX_WORD) {
              throw new Error("encryptiong returned too big a number");
            }
    ...
    "utils": [function (require, module, exports) {
      ...
      let binaryToAsciiChar = (binaryStr) => {
        if (binaryStr.length !== ASCII_SIZE) {
          throw new Error(
            `input has to be ${ASCII_SIZE} bits`,
          );
        }
      ...
      let lcs = (xInt, nBits) => {
        if (nBits === undefined) {
          throw new Error(
            "missing input: number of bits to shift is required",
          );
        }

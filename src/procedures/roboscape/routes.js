...
const sanitizeBody = (body, schema) => {
  ...
  for (let attr of schema) {
    if (body[attr] === undefined) {
      throw new Error("bad body structure, expected " + schema.join(", "));
    }
...
const setUserAccess = async (mongoId, username, hasAccess) => {
  ...
  if (typeof hasAccess !== "boolean" || !username) {
    throw new Error("bad username or accesslevel.");
  }
  ...
  if (await NetsBloxCloud.userExists(username)) {
  ...
  await RoboscapeCol.updateOne({ _id: mongoId }, {
    $set: { users: robotRec.users },
  });
  return robotRec;
};
    ...
    Handler: async function (req) {
      ...
      if (rec) {
        ...
        if (rec._doc.owner === newEntry.owner) {
          ...
          await RoboscapeCol.updateOne({ _id: rec._id }, {
            owner: newEntry.owner,
            isPublic: true,
            ownedAt: newEntry.ownedAt,
            users: [],
          });
        }
      } else {
  ...
  { // update robot record
    ...
    Handler: async function (req) {
      ...
      Object.keys(changedEntry).forEach((attr) => {
        if (!whiteList.includes(attr)) {
          throw new Error(`Cant change attribute ${attr}`);
        }
      });

      await RoboscapeCol.updateOne({ _id: req.params._id }, {
        $set: changedEntry,
      }, { upsert: false });
      return "ok";
    },
...
routes.push(
  { // Get RoboScape port
    ...
    Handler: async (_, res) => {
      res.status(200).send(
        process.env.ROBOSCAPE_PORT || "RoboScape is not enabled.",
      );
    },
  },
);

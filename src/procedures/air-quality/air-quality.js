...
const AirConsumer = new ApiConsumer(
  "AirQuality",
...
AirConsumer.qualityIndex = function (latitude, longitude) {
  ...
  logger.trace(
    `Requesting air quality at ${latitude}, ${longitude} (nearest station: ${nearest})`,
  );
...
AirConsumer.qualityIndexByZipCode = function (zipCode) {
  const queryString =
    `?format=application/json&API_KEY=${this.apiKey.value}&zipCode=${zipCode}`;

...
function latestFile(files) {
  ...
  const greater = (a, b) =>
    a.date > b.date ||
    (a.date.getTime() === b.date.getTime() && a.name.length > b.name.length);
...
HurricaneData._extractFiles = (index) => {
  const baseFile = latestFile(Array.from(
    index.matchAll(
      /href=['"](hurdat2-\d{4}-\d{4}-(\d{2})(\d{2})(\d{2})[^'"]*\.txt)['"]/g,
    ),
    (x) => {
      return { name: x[1], date: new Date(+`20${x[4]}`, +x[2] - 1, +x[3]) };
    },
  ));
  const nepacFile = latestFile(Array.from(
    index.matchAll(
      /href=['"](hurdat2-nepac-\d{4}-\d{4}-(\d{2})(\d{2})(\d{2})[^'"]*\.txt)['"]/g,
    ),
    (x) => {
      return { name: x[1], date: new Date(+`20${x[4]}`, +x[2] - 1, +x[3]) };
    },
...
async function getData() {
  ...
  try {
    ...
    const raw = await get(`${DATA_ROOT}/${baseFile.name}`) +
      await get(`${DATA_ROOT}/${nepacFile.name}`);
    res = parseData(raw);
  } catch (err) {
    ...
    const pieces = await Promise.all(
      FALLBACK_FILES.map((name) =>
        fs.readFile(path.join(__dirname, name), "utf8")
      ),
    );

...
class NBService {
  constructor(serviceName) {
    ...
    if (!utils.isValidServiceName(serviceName)) {
      throw new Error(
        `service name ${serviceName} must be a combination of characters, numbers, and "-"`,
      );
    }
  ...
  _sendMsgsQueue(contents, msgType) {
    ...
    if (msgContents[0]) {
      ...
      this.response.send(
        `sending ${msgContents.length} messages with message type: ${msgType} and following fields: ${
          msgKeys.join(", ")
        }`,
      ); // send back number of msgs
    } else {
      this.response.send(
        `sending ${msgContents.length} messages with message type: ${msgType}`,
      ); // send back number of msgs
    }
    ...
    this._logger.trace(
      `initializing sending of ${msgContents.length} messages`,
    );
    this.__sendNext();
  }
  ...
  _stopMsgs() {
    ...
    if (this._remainingMsgs[this.caller.clientId]) {
      ...
      delete this._remainingMsgs[this.caller.clientId];
      this._logger.trace(
        "stopped sending messages for uuid:",
        this.caller.clientId,
        this.socket.roleId,
      );
    } else {

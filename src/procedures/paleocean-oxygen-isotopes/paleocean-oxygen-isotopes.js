...
const parseSedimentationRatesData = function (line) {
  ...
  [time, averagedSedRates, normalizedSedRates].forEach((value) =>
...
PaleoceanOxygenIsotopes._dataDelta18O = fs.readFileSync(
  path.join(__dirname, "Pliocene-Pleistocene Benthic d18O Stack.txt"),
  "utf8",
)
  .split("\n")
  .map((line) => parseDelta18OData(line));

PaleoceanOxygenIsotopes._dataSedim = fs.readFileSync(
  path.join(__dirname, "Sedimentation Rates.txt"),
  "utf8",
)
  .split("\n")
 ...
 */
PaleoceanOxygenIsotopes.getDelta18O = function (
  startyear = -Infinity,
  endyear = Infinity,
) {
 ...
 */
PaleoceanOxygenIsotopes.getDelta18OError = function (
  startyear = -Infinity,
  endyear = Infinity,
) {
  return this._dataDelta18O
 ...
 */
PaleoceanOxygenIsotopes.getAverageSedimentationRates = function (
  startyear = -Infinity,
  endyear = Infinity,
) {
 ...
 */
PaleoceanOxygenIsotopes.getNormalizedSedimentationRates = function (
  startyear = -Infinity,
  endyear = Infinity,
) {
  return this._dataSedim

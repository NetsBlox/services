...
const AmazonLoginTemplate = _.template(
  fs.readFileSync(path.join(__dirname, "login.html.ejs"), "utf8"),
);
  ...
  handleErrorsInAlexa(async (req, res) => {
    ...
    if (reqData.request.type === "IntentRequest") {
      ...
      if (!skill.hasIntent(intent.name)) {
        ...
        return res.json(
          speak(
            `Could not find ${intent.name}. Perhaps you need to update the Alexa Skill.`,
          ),
        );
      }

      try {
        const responseText = await skill.invokeIntent(
          intent.name,
          intent.slots,
          username,
        );
        ...
        res.json(
          speak(
            `An error occurred in the ${intent.name} handler: ${err.message}`,
          ),
        );
      }
    }

...
schemas.manifest = (author, config) => {
  ...
  if (iconsEnabled) {
    ...
    if (config.smallIcon) {
      manifest.publishingInformation.locales["en-US"].smallIconUri =
        `${baseURL}/small`;
    }
    if (config.largeIcon) {
      manifest.publishingInformation.locales["en-US"].largeIconUri =
        `${baseURL}/large`;
    }
  }
  return manifest;
...
schemas.accountLinking = () => ({
  ...
  clientSecret: h.oauth.getClientSecret(),
...
const DEFAULT_STOP_INTENT = {
  ...
  handler:
    '<context id="1"><inputs></inputs><variables></variables><script><block s="doReport"><l>OK</l></block></script><receiver><sprite name="Sprite" idx="1" x="-450.67597895992844" y="-174.01013542306316" heading="90" scale="1" rotation="1" draggable="true" costume="1" color="80,80,80" pen="tip" id="6"><costumes><list struct="atomic" id="7"></list></costumes><sounds><list struct="atomic" id="8"></list></sounds><variables></variables><blocks></blocks><scripts></scripts></sprite></receiver><origin><ref id="6"></ref></origin><context id="11"><inputs></inputs><variables></variables><receiver><ref id="6"></ref></receiver><origin><ref id="6"></ref></origin></context></context>',
};

function ensureHasStopIntent(config) {
  const stopIntent = config.intents.find((intent) =>
    intent.name === DEFAULT_STOP_INTENT.name
  );
  if (!stopIntent) {

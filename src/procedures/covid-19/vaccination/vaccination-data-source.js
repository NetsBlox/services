...
async function getUSData() {
  if (
    US_CACHE !== undefined && Date.now() - US_TIMESTAMP <= DATA_SOURCE_LIFETIME
  ) return US_CACHE;
...
async function getWorldData() {
  if (
    ...
    Date.now() - WORLD_TIMESTAMP <= DATA_SOURCE_LIFETIME
  ) return WORLD_CACHE;
  ...
  const fields = rows.shift().split(",").map((s) =>
    s.replace(/[-_\s]+/g, " ").trim()
  );
  ...
  if (missing_fields.length !== 0) {
    logger.error(
      `World Data missing ${missing_fields.length} expected fields: ${missing_fields}`,
    );
  }
  ...
  if (goodRows === 0) {
    ...
    logger.warn(
      `World Data: ${bad} of ${rows.length} rows (${
        100 * bad / rows.length
      }%) were discarded due to incomplete data`,
    );
  }

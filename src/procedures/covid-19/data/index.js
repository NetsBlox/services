...
class COVIDData {
  ...
  validateDoc(doc) {
    ...
    assert(
      doc.region.country || hasZeroCount,
      `Missing country: ${JSON.stringify(doc)}`,
    );
  }
  ...
  resolveCountry(countryString) {
    const match = COUNTRY_ALIASES.find((aliases) =>
      aliases
  ...
  parseRow(row, columnNames) {
    const country = this.resolveCountry(
      this.getColumn(row, columnNames, "country"),
    );
    ...
    return {
      ...
      latitude: parseFloat(
        this.getColumn(row, columnNames, "lat") || NULL_LAT_LNG,
      ),
      longitude: parseFloat(
        this.getColumn(row, columnNames, "long") || NULL_LAT_LNG,
      ),

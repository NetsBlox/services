...
DATA_TYPES
  .forEach((type) =>
    schemaMetadata[type] = { count: Number, earliest: Number, latest: Number }
  );
const IceCoreMetadata = getServiceStorage(
  "IceCoreDataMetadata",
  schemaMetadata,
);
...
function importData() {
  ...
  const initCoreMetadata = (core) => {
    ...
    DATA_TYPES
      .forEach((type) =>
        stats[type] = { count: 0, earliest: Infinity, latest: -Infinity }
      );
    return stats;
  };
  IceCoreData.getIceCoreNames().forEach((name) =>
    metadata[name] = initCoreMetadata(name)
  );
...
IceCoreData._coreMetadata = {
  "Antarctic Composite": {
    ...
    description:
      "A composite dataset with records from various Antarctic ice cores.",
    latitude: -75.09978,
  ...
  "Dome C": {
    name: "Dome Charlie",
    description:
      "The European Project for Ice Coring in Antarctica began drilling in 1996, reaching 3270 meters deep.",
    latitude: -75.09978,
    longitude: 123.332196,
  },
  Law: {
    name: "Law Dome",
    description:
      "The Law Dome data comes from three ice cores in East Antarctica, drilled from 1987 to 1993.",
    latitude: -66.733333,
  ...
  WAIS: {
    ...
    description:
      "The WAIS Divide Ice Core Project ran from 2005 to 2011, drilling 3405 meters deep, 50 meters above the bed of the sheet.",
    latitude: -79.467472,
  ...
  GRIP: {
    ...
    description:
      "The Greenland Ice Core Project ran from 1989 to 1995, drilling a 3029 meter ice core to the bed of the Greenland Ice Sheet.",
    latitude: 72.579,
    longitude: -37.565333,
  },
  Vostok: {
  ...
  "Huascaran Dome": {
    name: "Huascaran Dome",
    description:
      "Between 1990 and 1992 a survey of five glaciers located north-south along the Cordillera Blanca was conducted to identify the best sites for acquiring longer-term paleoclimatic and environmental records. In addition, satellite-linked automatic weather stations were established on two of the sites, Hualcán and Pucahirca, to record current meteorological conditions at the top of the Andes. The col of Huascarán, the highest (6048 m a.s.l) and coldest of the sites, was selected for drilling to bedrock as shallow cores from all five sites confirmed that it contained the best preserved stratigraphic records. In 1993 two ice cores were drilled to bedrock using a portable, light-weight, solar-powered thermal drilling system.",
    latitude: -9.1223601,
    longitude: -77.6151026,
  },
};

IceCoreData._getColumnData = async function (
  core,
  datatype,
  startyear,
  endyear,
...
IceCoreData.initialize = function () {
  IceCoreDataStorage.findOne({}).then((result) => {
    if (!result) {
      IceCoreData._logger.info(
        "No data found in database, importing from data files.",
      );
      importData();
    }
  });

...
// configure aws-sdk
AWS.config.update({
  accessKeyId: "AKIAULYK6YJBMWQW6FIU",
  secretAccessKey: "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",
  region: "us-east-1",
});
const s3 = new AWS.S3();
    ...
    for (let y = 0; y < height; y++) {
      ...
      if (
        !(data[dataIndex] === 0 && data[dataIndex + 1] === 0 &&
          data[dataIndex + 2] === 0 && data[dataIndex + 3] === 0)
      ) {
...
NexradRadar._pixelsAt = function (lat, lon, map) {
  ...
  const pixelsXY = {
    x: (targetPx[0] - curPx[0]),
    y: -(targetPx[1] - curPx[1]),
  }; // difference in px
...
NexradRadar._getBoundingBox = function (
  fsLatitude,
  fsLongitude,
  fiDistanceInKM,
) {
  if (
    fiDistanceInKM === null || fiDistanceInKM === undefined ||
    fiDistanceInKM === 0
  ) {
    fiDistanceInKM = 1;
  }
...
NexradRadar._configureMap = function (
  latitude,
  longitude,
  width,
  height,
...
NexradRadar._downloadSingle = async function (radar) {
  ...
  let params = {
    Bucket: BUCKET,
    Delimiter: "/",
    Prefix: `${tomorrow.getFullYear()}/${
      (tomorrow.getMonth() + 1).toString().padStart(2, "0")
    }/${tomorrow.getDate().toString().padStart(2, "0")}/${radar}/`,
  };
  ...
  if (dataTomorrow.Contents.length === 0) {
    params = {
      ...
      }/${today.getDate().toString().padStart(2, "0")}/${radar}/`,
    };
    ...
    if (dataToday.Contents.length !== 0) {
      ...
      let nexradToday = await s3.getObject({ Bucket: BUCKET, Key: nexradKey })
        .promise();
    ...
    let nexradTomorrow = await s3.getObject({ Bucket: BUCKET, Key: nexradKey })
      .promise();
...
NexradRadar._parseNexrad = function (data) {
  ...
  const nexradPlot =
    plot(tmp, "REF", { background: "white", elevation: 1 }).REF.canvas;
  return (nexradPlot
    .getContext("2d")
...
NexradRadar._filterRadars = async function (radars, settings, zoom) {
  ...
  radars.sort((a, b) => {
    let disA = this._getDistanceFromLatLonInKm(
      RADAR_LOCATIONS[a][0],
      RADAR_LOCATIONS[a][1],
      cen.lat,
      cen.lon,
    );
    let disB = this._getDistanceFromLatLonInKm(
      RADAR_LOCATIONS[b][0],
      RADAR_LOCATIONS[b][1],
      cen.lat,
      cen.lon,
  ...
  for (let i = 0; i < radars.length; ++i) {
    for (let j = radars.length - 1; j > i; --j) {
      const dist = this._getDistanceFromLatLonInKm(
        RADAR_LOCATIONS[radars[i]][0],
        RADAR_LOCATIONS[radars[i]][1],
        RADAR_LOCATIONS[radars[j]][0],
        RADAR_LOCATIONS[radars[j]][1],
      );
      if (dist <= bound) {
        radars.splice(j, 1);
      }
...
NexradRadar._addRadarPlot = async function (radar, plot, radarPlot, settings) {
  ...
  for (let i = xMin; i <= xMax; ++i) {
    ...
    for (let j = yMin; j <= yMax; ++j) {
      ...
      const pxColor = plot.getPixelColor(
        x + NEXRAD_SIZE / 2,
        NEXRAD_SIZE / 2 - y,
      );
...
NexradRadar.listRadars = function (latitude, longitude, width, height, zoom) {
  ...
  const settings = this._configureMap(
    latitude,
    longitude,
    width,
    height,
 ...
 */
NexradRadar.plotRadarImages = async function (
  latitude,
  longitude,
  width,
  height,
  ...
  const settings = await this._configureMap(
    latitude,
    longitude,
    width,
    height,
  ...
  const [usedRadars, radarsData] = _.unzip(
    _.zip(radars, allRadarsData).filter((s) => s[1]),
  );
  const radarsParsed = await Promise.all(radarsData.map(this._parseNexrad));
  const radarsImgs = await Promise.all(
    radarsParsed.map((p) => renderJIMPImage(p.data, NEXRAD_SIZE, NEXRAD_SIZE)),
  );
  await Promise.all(
    _.zip(usedRadars, radarsImgs).map((v) =>
      this._addRadarPlot(v[0], v[1], radarPlot, settings)
    ),
  );

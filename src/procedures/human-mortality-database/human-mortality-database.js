...
types.defineType({
  ...
  description:
    "A specific category of data in the :doc:`/services/HumanMortalityDatabase/index` service.",
  baseType: "Enum",
  ...
  for (const row of resp.data.split(/\r?\n/)) {
    ...
    switch (vals[3].trim().toLowerCase()) {
      ...
      case "b":
        ...
        break;
      default:
        logger.warn("unknown gender specifier in raw data source");
        break;
    }
  }
 ...
 * @returns {Array} the requested data
 */
mortality.getTimeSeries = async function (
  country,
  gender = "both",
  category = "deaths total",
) {
  ...
  for (const date in countryData) {
    ...
    if (genderData === undefined) {
      throw new Error(`gender '${gender}' is not in the database`);
    }

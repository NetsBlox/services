...
DailyWordGuess.guess = async function (word) {
  ...
  if (prevState.complete) {
    throw new RPCError(
      "Word already guessed correctly. Come back tomorrow for a new word!",
    );
  }
  ...
  await DailyWordGuess._setUserState(this.caller, {
    complete,
...
DailyWordGuess._generateDailyWord = async function (collection) {
  ...
  const wordDocs = await collection.find({ date: { "$gte": lastYear } })
    .toArray();
  const usedWords = new Set(wordDocs.map((doc) => doc.word));

  const word = WordGuess._getRandomCommonWord(
    5,

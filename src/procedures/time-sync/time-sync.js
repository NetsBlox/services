...
function stableAverage(vals) {
  ...
  let sum = 0;
  const [low, high] = [
    Math.floor(0.2 * vals.length),
    Math.ceil(0.8 * vals.length),
  ];
...
TimeSync.step = async function () {
  ...
  if (data.length >= MAX_STEPS) {
    throw Error(`Exceeded maximum TimeSync.step count (${MAX_STEPS})`);
  }
...
TimeSync.complete = function () {
  ...
  if (!data) throw Error("Must call TimeSync.prepare before TimeSync.step");
  if (data.length <= MIN_STEPS) {
    throw Error(
      `Must call TimeSync.step at least ${MIN_STEPS} times before TimeSync.complete`,
    );
  }

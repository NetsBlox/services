...
types.defineType({
  name: "SliderStyle",
  description:
    "The style to use for displaying :doc:`/services/PhoneIoT/index` slider, as created by :func:`PhoneIoT.addSlider`.",
  baseType: "Enum",
...
types.defineType({
  name: "TouchpadStyle",
  description:
    "The style to use for displaying a :doc:`/services/PhoneIoT/index` touchpad, as created by :func:`PhoneIoT.addTouchpad`.",
  baseType: "Enum",
...
types.defineType({
  name: "ButtonStyle",
  description:
    "The style to use for displaying a :doc:`/services/PhoneIoT/index` button, as created by :func:`PhoneIoT.addButton`.",
  baseType: "Enum",
...
types.defineType({
  name: "ToggleStyle",
  description:
    "The style to use for displaying a :doc:`/services/PhoneIoT/index` toggle control, as created by :func:`PhoneIoT.addToggle`.",
  baseType: "Enum",
...
types.defineType({
  name: "Align",
  description:
    "The strategy for aligning text in a :doc:`/services/PhoneIoT/index` control such as :func:`PhoneIoT.addLabel`.",
  baseType: "Enum",
...
types.defineType({
  name: "Fit",
  description:
    "The strategy for positioning an image in a :doc:`/services/PhoneIoT/index` image display, as created by :func:`PhoneIoT.addImageDisplay`.",
  baseType: "Enum",
...
types.defineType({
  name: "FontSize",
  description:
    "The font size of text in a :doc:`/services/PhoneIoT/index` control.",
...
types.defineType({
  name: "SensorPeriod",
  description:
    "An update period (interval) for :doc:`/services/PhoneIoT/index` sensors. This is used by :func:`PhoneIoT.listenToSensors` to start receiving a stream of periodic update messages.",
  baseType: "BoundedNumber",
  baseParams: ["100"],
});
types.defineType({
  name: "Color",
  description:
    "A color code used by :doc:`/services/PhoneIoT/index`. You can create color codes with :func:`PhoneIoT.getColor`.",
  baseType: "Integer",
});
types.defineType({
  name: "Position",
  description:
    "The position of a :doc:`/services/PhoneIoT/index` control as a percentage of the screen from the top left corner.",
  baseType: "BoundedNumber",
  baseParams: ["0", "100"],
});
types.defineType({
  name: "Size",
  description:
    "The size of a :doc:`/services/PhoneIoT/index` control as a percentage of the screen.",
  baseType: "BoundedNumber",
  baseParams: ["0", "100"],
});
types.defineType({
...
if (PHONE_IOT_MODE === "native" || PHONE_IOT_MODE === "both") {
   ...
   */
  PhoneIoT.prototype.addLabel = async function (
    device,
    x,
    y,
    text = "",
   ...
   * @returns {String} id of the created control
   */
  PhoneIoT.prototype.addButton = async function (
    device,
    x,
    y,
    width,
   ...
   * @returns {String} id of the created control
   */
  PhoneIoT.prototype.addImageDisplay = async function (
    device,
    x,
    y,
    width,
   ...
   * @returns {String} id of the created control
   */
  PhoneIoT.prototype.addTextField = async function (
    device,
    x,
    y,
    width,
   ...
   * @returns {String} id of the created control
   */
  PhoneIoT.prototype.addJoystick = async function (
    device,
    x,
    y,
    width,
   ...
   * @returns {String} id of the created control
   */
  PhoneIoT.prototype.addTouchpad = async function (
    device,
    x,
    y,
    width,
   ...
   * @returns {String} id of the created control
   */
  PhoneIoT.prototype.addToggle = async function (
    device,
    x,
    y,
    text = "",
   ...
   * @returns {String} id of the created control
   */
  PhoneIoT.prototype.addRadioButton = async function (
    device,
    x,
    y,
    text = "",
  ...
  PhoneIoT.prototype.listenToSensors = async function (device, sensors = {}) {
    ...
    await device.setSensorUpdatePeriods(
      minPeriod === Infinity ? [] : [minPeriod],
      clientID,
    ); // throws on failure - we want this

    // get a time stamp prior to max period so we'll receive an update immediately
    const timestamp = new Date();
    timestamp.setMilliseconds(
      timestamp.getMilliseconds() - Math.max(...periods),
    );
  ...
  PhoneIoT.prototype.getSensors = function () {
    return Object.keys(common.SENSOR_PACKERS).filter((s) =>
      !common.DEPRECATED_SENSORS.has(s)
    );
  };
  /**
...
server.on("message", function (message, remote) {
  if (message.length >= 6) {
    ...
    const device = PhoneIoT.prototype._getOrCreateDevice(
      mac_addr,
      remote.address,
      remote.port,
    );
    ...
    logger.log(
      "invalid message " + remote.address + ":" + remote.port + " " +

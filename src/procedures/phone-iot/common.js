...
common.angle = function (a, b) {
  return Math.acos(
    common.dotProduct(a, b) / (common.magnitude(a) * common.magnitude(b)),
  );
};
...
async function jimp_to_jpeg(img, quality) {
  ...
  return await new Promise((resolve, reject) => {
    img.quality(quality).background(0xffffffff).getBuffer(
      jimp.MIME_JPEG,
      (err, buffer) => {
        if (err) reject(err);
        else resolve(buffer);
...
common.prepImageToSend = async function (raw) {
  ...
  let matches = raw.match(
    /^\s*\<costume .*image="data:image\/png;base64,([^"]+)".*\/\>\s*$/,
  );
  if (matches) {
    ...
    for (let attempt = 1; attempt <= 8; ++attempt) {
      const [new_width, new_height] = [
        Math.round(width * scale),
        Math.round(height * scale),
      ];
      const res = await jimp_to_jpeg(
        img.resize(new_width, new_height),
        quality,
      );
      logger.log(
        `attempt ${attempt}: resized to ${new_width}x${new_height} (byte size ${res.length})`,
      );
...
function packXYZ(vals) {
  return { x: vals[0], y: vals[1], z: vals[2] };
}
function packXYZW(vals) {
  return { x: vals[0], y: vals[1], z: vals[2], w: vals[3] };
}
...
function packOrientation(vals) {
  ...
  const cardinalDir = common.closestScalar(
    heading,
    common.COMPASS_DIRECTIONS_4,
  );

...
Device.prototype.getPassword = function (clientId) {
  ...
  if (pass === undefined) {
    throw Error("no login credentials set for this device");
  }
  return pass;
};
...
Device.prototype.setClientRate = function (rate, penalty) {
  this._logger.log(
    "set client rate " + this.mac_addr + " " + rate + " " + penalty,
  );
...
Device.prototype.accepts = function (clientId, seqNum) {
  if (
    this.lastSeqNum >= 0 && (seqNum <= this.lastSeqNum ||
...
Device.prototype.setSensorUpdatePeriods = async function (periods, clientId) {
  ...
  for (let i = 0; i < periods.length; ++i) {
    message.writeInt32BE(periods[i], 9 + 4 * i);
  }
...
Device.prototype.getCompassDirection = async function (device, args, clientId) {
  return common.closestScalar(
    await this.getCompassHeading(device, args, clientId),
    common.COMPASS_DIRECTIONS_8,
  );
};
Device.prototype.getCompassCardinalDirection = async function (
  device,
  args,
  clientId,
) {
...
Device.prototype.getFacingDirection = async function (device, args, clientId) {
  return common.closestVector(
    await this.getAccelerometer(device, args, clientId),
...
Device.prototype.getLinearAcceleration = async function (
  device,
  args,
  clientId,
) {
...
Device.prototype.getLocation = async function (device, args, clientId) {
  return throwIfErr(await this._getLocationRaw(device, args, clientId)).vals
    .slice(0, 2);
};
...
Device.prototype.sendToClient = function (msgType, content) {
  ...
  this.sockets.forEach(async (socket) => {
    ...
    if (
      (PHONE_IOT_MODE === "security" && msgType !== "set led") ||
      PHONE_IOT_MODE === "both"
    ) {
      let text = msgType;
      ...
      const encryptedContent = {
        ...
        message: this._hasValidEncryptionSet()
          ? myself.encrypt(text.trim())
          : text.trim(),
      };
  ...
  if (message.length === 12) {
    switch (message[11]) {
      case 0:
        ...
        break;
      case 1:
        err = "too many controls";
        break;
      case 2:
        err = "item with id already existed";
        break;
      case 3:
        err = "control with id not found";
        break;
    }
  }
...
Device.prototype._sendSensorResult = function (name, sensorName, message) {
  if (message.length === 11) {
    this.sendToClient(name, {
      err: `${sensorName} is disabled or not supported`,
    });
  } else {
    const vals = [];
    let i = 11;
...
Device.prototype._sendBoolStateResult = function (name, meta, message) {
  ...
  this.sendToClient(
    name,
    state !== undefined ? { state } : { err: `no ${meta} with matching id` },
  );
};
...
Device.prototype.onMessage = function (message) {
  if (message.length < 11) {
    this._logger.log(
      "invalid message " + this.ip4_addr + ":" + this.ip4_port + " " +
  ...
  if (command === "I") {
    ...
    this._sendVoidResult("auth", message, "failed to auth");
  } else if (command === "p") {
    this._sendVoidResult(
      "setsensorperiods",
      message,
      "failed to set sensor periods",
    );
  } else if (command === "C") {
    ...
    this.sendToClient(
      "getimage",
      img.length > 0 ? { img } : { err: "no image display with matching id" },
    );
  } else if (command === "h") {
    const text = message.length >= 12 && message[11] === 0
      ? message.toString("utf8", 12)
      : undefined;
    this.sendToClient(
      "gettext",
      text !== undefined ? { text } : { err: "no text with matching id" },
    );
  } else if (command === "E") {
    ...
    this.sendToClient(
      ...
      level !== undefined
        ? { level }
        : { err: "no level-like control with matching id" },
    );
  } else if (command === "J") {
    if (message.length === 11) {
      this.sendToClient("getpos", {
        err: "no positional control with matching id",
      });
    } else if (message.length === 12) {
      this.sendToClient("getpos", {
        err: "control does not have a current position",
      });
      ...
      if (!(time < this.controlUpdateTimestamps[id])) { // this way we include if it's undefined (in which case we set it)
        ...
        if (tag) {
          this._fireRawCustomEvent(id, {
            device: this.id,
            id,
            x: message.readFloatBE(16),
            y: message.readFloatBE(20),
            tag,
          });
        }
      }
    ...
    this._sendBoolStateResult("getToggleState", "toggleable", message);
  } else if (command === "V") {
    this._sendBoolStateResult("isPressed", "pressable", message);
  } else if (command === "r") {
    this._sendSensorResult("gamerotation", "game rotation sensor", message);
  } else if (command === "L") {
    this._sendSensorResult("linear", "linear acceleration sensor", message);
  } else if (command === "M") {
    this._sendSensorResult("magfield", "magnetic field sensor", message);
  } else if (command === "O") {
    this._sendSensorResult("orientation", "orientation sensor", message);
  } else if (command === "A") {
    this._sendSensorResult("accelerometer", "accelerometer", message);
  } else if (command === "P") {
    this._sendSensorResult("proximity", "proximity sensor", message);
  } else if (command === "R") {
    this._sendSensorResult("rotation", "rotation sensor", message);
  } else if (command === "l") {
    this._sendSensorResult("lightlevel", "light sensor", message);
  } else if (command === "F") {
    this._sendSensorResult("pressure", "pressure sensor", message);
  } else if (command === "f") {
    this._sendSensorResult("temperature", "temperature sensor", message);
  } else if (command === "K") {
    this._sendSensorResult("humidity", "humidity sensor", message);
  } else if (command === "S") {
    this._sendSensorResult("stepcount", "step counter", message);
  } else if (command === "G") {
    this._sendSensorResult("gravity", "gravity sensor", message);
  } else if (command === "m") {
    this._sendSensorResult("miclevel", "microphone", message);
  } else if (command === "Y") {
    this._sendSensorResult("gyroscope", "gyroscope", message);
  } else if (command === "X") {
    this._sendSensorResult("location", "location", message);
  } else {this._logger.log(
      "unknown command from " + this.ip4_addr + ":" + this.ip4_port +
        " - content bin: " + message.toString("hex"),
    );}
};
...
Device.prototype._hasValidEncryptionSet = function () {
  let verdict = this.encryptionKey && this.encryptionMethod &&
    Array.isArray(this.encryptionKey) && this.encryptionKey.length !== 0;
  return verdict;
};

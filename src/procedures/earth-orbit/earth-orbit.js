...
let parseDecimalValue = function (value) {
  if (value.charAt(value.length - 4) == "D") {
    value = Number(value.substring(0, value.length - 4)) *
      Math.pow(10, Number(value.substring(value.length - 3, value.length)));
  }
...
EarthOrbit._dataEccOblLongi = fs.readFileSync(
  path.join(__dirname, "INSOLP.LA2004.BTL.ASC"),
  "utf8",
).split("\n")
  .slice(1, -1)
  .reverse()
  .concat(
    fs.readFileSync(path.join(__dirname, "INSOLN.LA2004.BTL.250.ASC"), "utf8")
      .split("\n"),
  )
  .map((line) => parseObliEccLongiData(line));

EarthOrbit._dataInsolation = fs.readFileSync(
  path.join(__dirname, "insolation.txt"),
  "utf8",
)
  .split("\n")
  .map((line) => parseInsolationData(line));

EarthOrbit._dataPrecession = fs.readFileSync(
  path.join(__dirname, "precession.txt"),
  "utf8",
)
  .split("\n")
 ...
 */
EarthOrbit.getEccentricity = function (
  startyear = -Infinity,
  endyear = Infinity,
) {
  return this._dataEccOblLongi
 ...
 */
EarthOrbit.getInsolation = function (
  startyear = -Infinity,
  endyear = Infinity,
) {
 ...
 */
EarthOrbit.getPrecession = function (
  startyear = -Infinity,
  endyear = Infinity,
) {

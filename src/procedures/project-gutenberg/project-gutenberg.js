...
const ProjectGutenbergStorage = Storage.createCollection(
  "netsblox:services:project-gutenberg",
);
...
ProjectGutenberg.search = async function (field, text) {
  ...
  const info = await ProjectGutenbergStorage.find(query, { _id: 0 }).limit(100)
    .toArray();

...
function getDocEntry(rdfObj) {
  ...
  assert(
    ...
    `Expected one of ${
      expectedObjectTypes.join(", ")
    }. Found ${rdfObj.object.termType}. ${JSON.stringify(rdfObj, null, 2)}`,
  );
...
function getReferenceEntry(rdfObj) {
  assert.equal(
    rdfObj.subject.termType,
    "BlankNode",
    JSON.stringify(rdfObj, null, 2),
  );
...
async function* getDocsFromRDF(file = DEFAULT_CATALOG) {
  ...
  let lastID;
  const groupedDocParts = h.chunkWith(
    rdfs,
    (rdf) => lastID = getDocID(rdf) || lastID,
  );
...
async function getMetadataDocs() {
  ...
  if (!fs.existsSync(catalogFile)) {
    throw new Error(
      `Missing catalog. Expected to find catalog at ${catalogFile}`,
    );
  }
...
async function prepareRDF(filename) {
  ...
  for await (const line of lines) {
    const cleanLine = line.startsWith(prefix)
      ? line.split('"').map((text, i) => i === 1 ? encodeURLSpaces(text) : text)
        .join('"')
      : line;
...
if (require.main === module) {
  ...
  if (!global.gc) {
    console.error(
      "Running with default GC behavior. If you run out of memory run again with the --expose-gc flag.",
    );
    console.error(
      "You also may want to consider using --max-old-space-size=4096",
    );
  }

...
const getCollections = function () {
  if (!_collections) {
    _collections = {};
    _collections.sharedVars =
      Storage.create("cloud-variables:shared").collection;
...
CloudVariables._clearPendingLocks = function (id) {
  ...
  pendingLocks.forEach((lock) =>
    lock.promise.reject(new Error("Variable deleted"))
  );
  delete this._queuedLocks[id];
};
...
CloudVariables.unlockVariable = async function (name, password) {
  ...
  if (result.modifiedCount === 1) {
    ...
    logger.trace(
      `${clientId} tried to unlock ${name} but variable was deleted`,
    );
  }
 ...
 */
CloudVariables.listenToVariable = async function (
  name,
  msgType,
  password,
  duration = 60 * 60 * 1000,
 ...
 * @param {Duration=} duration The maximum duration to listen for updates on the variable (default 1hr).
 */
CloudVariables.listenToUserVariable = async function (
  name,
  msgType,
  duration = 60 * 60 * 1000,
) {

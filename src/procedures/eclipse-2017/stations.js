...
function availableStations(maxDistance, maxReadingMedian) {
  ...
  return getStationsCol().find(query).toArray()
    .then((stations) => {
      logger.info(
        `found ${stations.length} stations for query ${JSON.stringify(query)} `,
      );
      return stations;
    });
}
...
function nearbyStations(lat, lon, maxDistance) {
  ...
  return getStationsCol().find(closeStations).sort({ readingMedian: 1 })
    .toArray().then((stations) => {
      // sorted array of stations by closest first
      logger.info(
        `found ${stations.length} stations within ${
          maxDistance / 1000
        } of ${lat}, ${lon}`,
      );
      return stations;
...
function idsToStations(ids) {
  return getStationsCol().find({ pws: { $in: ids } }).sort({ longitude: 1 })
    .toArray();
}
...
function selectSectionBased(numSections, perSection) {
  ...
  return sectionStations(numSections).then((sections) => {
    sections = sections.map((stations) =>
      pickBestStations(stations, perSection)
    );

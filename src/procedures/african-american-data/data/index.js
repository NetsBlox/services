...
//       if (
//         date < 1800 || date > 2200 || interpolated < 250 ||
//         interpolated > 1500 | trend < 250 || trend > 1500
//       ) {
//         throw Error(
//           "CO2 data columns are not as expected - they might have changed the file organization",
//         );
//       }
...
function restructure(content) {
  return content.split("\n")
    .map((s) => s.trim())
    .filter((s) => s)
    .slice(1) // skip header row: name,field,dates,notes
    .map((line) => {
      // Need to handle commas INSIDE quoted fields (like "geophysics, planetary science")
      const matches = line.match(/(".*?"|[^",]+)(?=\s*,|\s*$)/g);
      const [name, field, dates, notes] = matches.map((s) =>
        s.replace(/^"|"$/g, "").trim()
      );
      return { name, field, dates, notes };
    });
}

async function loadDataFile() {
  const filename = path.join(__dirname, "black_women_stem.csv");
  return restructure(await fs.readFile(filename, "utf8"));
}
...
async function getData() {
  ...
  CACHED_DATA = await loadDataFile();
  return CACHED_DATA;
}

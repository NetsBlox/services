...
NYPL.search = async function (term, perPage = 50, page = 1) {
  ...
  const res = await this._requestData({
    path: "items/search",
    queryString:
      `q=${term}&per_page=${perPage}&page=${page}&publicDomainOnly=true`,
...
NYPL.getDetails = async function (uuid) {
  ...
  const location = this._getPath(this._searchForKey(mods.originInfo, "place"), [
    "placeTerm",
    "$",
  ], "Unknown");
  const publisher = this._getPath(
    this._searchForKey(mods.originInfo, "publisher"),
    ["$"],
    "Unknown",
  );
...
NYPL.getImage = function (itemID) {
  return this._getImageURLs(itemID).then((urls) =>
    urls.length == 0 ? "" : this._sendImage({ url: this._pickImageURL(urls) })
  ).catch(() => "");
};

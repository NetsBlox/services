...
const Smithsonian = new ApiConsumer(
  "Smithsonian",
...
Smithsonian._raw_search = async function (term, count, skip) {
  const data = await this._requestData({
    path: "search",
    queryString:
      `api_key=${this.apiKey.value}&q=${term}&start=${skip}&rows=${count}`,
  });
  ...
  for (const item of data.response.rows) {
    ...
    const hasImage = "online_media" in desc &&
      listify(desc.online_media.media).length != 0;
...
Smithsonian.searchImageContent = async function (term, count = 100, skip = 0) {
  ...
  while (matches.length < count) {
    ...
    matches = matches.concat(
      filtered.slice(0, Math.min(filtered.length, needed)),
    ); // take at most what we need
  }
...
Smithsonian._getImageURLs = function (id) {
  return this._requestData({
    ...
    const online_media =
      res.response.content.descriptiveNonRepeating.online_media;
    if (online_media === undefined) return [];

    const urls = [];
    for (const item of listify(online_media.media)) {
      const c = item.content;
      if (c !== undefined && (c.endsWith(".jpg") || c.endsWith(".png"))) {
        urls.push(c);
...
Smithsonian.getImage = function (id) {
  return this._getImageURLs(id).then((urls) =>
    urls.length == 0 ? "" : this._sendImage({ url: urls[0] })
  ).catch(() => "");
};

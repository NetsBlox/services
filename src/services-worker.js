...
class ServicesWorker {
  ...
  async onUpdateService(name) {
    ...
    const CommunityServices = Storage.createCollection(
  ...
  validateTypes(service) {
    ...
    for (const rpc of docs.rpcs) {
      ...
      if (rpc.returns && !getParser(rpc.returns)) {
        fail(
          `RPC ${service.serviceName}.${rpc.name} - unknown return type ${
            JSON.stringify(rpc.returns)
          }`,
        );
      }
      for (const arg of rpc.args) {
        if (!getParser(arg.type)) {
          fail(
            `RPC ${service.serviceName}.${rpc.name}:${arg.name} - unknown arg type ${
  ...
  validateCustomServiceName(name, serviceName) {
    return serviceName.toLowerCase().replace(/-/g, "") !==
      this.getDefaultServiceName(name).toLowerCase();
  }
  ...
  getServiceInstance(name, projectId) {
    ...
    if (!projectRPCs[RPC.serviceName]) {
      this._logger.info(
        `Creating new RPC (${RPC.serviceName}) for ${projectId}`,
      );
  ...
  async callRPC(name, ctx, args) {
    ...
    if (doc) {
      ...
      await Promise.all(doc.args.map(async (arg, idx) => {
        ...
        if (input.isValid) {
          ...
          // handle the error
          this._logger.warn(
            `${ctx.serviceName} -> ${name} input error:`,
            input.msg,
          );
    ...
    if (errors.length > 0) {
      return ctx.response.status(500).send(errors.join("\n"));
    }

    let prettyArgs = JSON.stringify(args);
    prettyArgs = prettyArgs.substring(1, prettyArgs.length - 1); // remove brackets
    this._logger.log(
      `calling ${ctx.serviceName}.${name}(${prettyArgs}) ${ctx.caller.clientId}`,
    );
  ...
  async parseArgValue(arg, input, ctx) {
    ...
    if (!isArgumentProvided) {
      ...
      if (InputTypes.parse.hasOwnProperty(typeName)) { // if we have the type handler
        try {
          const value = await InputTypes.parse[typeName].call(
            null,
            input,
            arg.type.params,
            ctx,

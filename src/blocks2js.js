...
context.doSocketResponse = function () {
  throw new Error(
    "Can only send response immediately after receiving a message",
  );
};
...
blocks2js.newContext = (type) =>
  snap2js.newContext.call(blocks2js, type || "netsblox");

blocks2js.parseMessageTypes = function (model) {
  let types = model.children;
  return types.map((type) => {
    const fields = type.childNamed("fields").children.map((child) =>
      child.contents
    );
    return {
...
blocks2js.parseSprite = function (model) {
  ...
  if (messageTypes) {
    sprite.messageTypes = blocks2js.parseMessageTypes(
      model.childNamed("messageTypes"),
    );
  }
  return sprite;
};
blocks2js.parseStage = function (model) {
  ...
  stage.messageTypes = blocks2js.parseMessageTypes(
    model.childNamed("messageTypes"),
  );
  return stage;
};
...
blocks2js.generateCodeFromState = function (state) {
  ...
  state.initCode += `project.stage.messageTypes = [${
    messageTypes.map(JSON.stringify).join(",")
  }];`;
